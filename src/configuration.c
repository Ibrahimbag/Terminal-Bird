...
static void assign_config(Configurations *config, cJSON *json)
{
	config->game_speed_multiplier =
		get_item(json, "General", "game_speed_multiplier")->valuedouble;
	config->show_score =
		cJSON_IsTrue(get_item(json, "General", "show_score"));
	config->menu_shown =
		cJSON_IsTrue(get_item(json, "General", "menu_shown"));
	config->jump_height =
		get_item(json, "General", "jump_height")->valueint;
	...
	config->bird_color =
		get_item(json, "Appearance", "bird_color")->valueint;
	config->pipe_color =
		get_item(json, "Appearance", "pipe_color")->valueint;
	config->background_color =
		get_item(json, "Appearance", "background_color")->valueint;
	config->auto_resize =
		cJSON_IsTrue(get_item(json, "Display", "auto_resize"));
